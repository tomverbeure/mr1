...
class TopVexRiscv(config: MR1Config) extends Component {
    ...
    val core = new ClockingArea(coreClockDomain) {
        ...
        if (false){
            ...
            iBus.rsp.ready := RegNext(iBus.cmd.valid) init(False)
            iBus.rsp.inst  := cpu_ram.readSync(
                    enable  = iBus.cmd.valid,
                    address = (iBus.cmd.payload.pc >> 2).resized
                )

            dBus.cmd.ready := True

            val wmask = dBus.cmd.payload.size.mux(
                            B"00"   -> B"0001",
                            B"01"   -> B"0011",
                            default -> B"1111") |<< dBus.cmd.address(1 downto 0)

            dBus.rsp.ready := RegNext(dBus.cmd.valid && !dBus.cmd.wr) init(False)
            dBus.rsp.data := cpu_ram.readWriteSync(
                    enable  = dBus.cmd.valid && !dBus.cmd.address(19),
                    address = (dBus.cmd.address >> 2).resized,
                    write   = dBus.cmd.wr,
                    data    = dBus.cmd.data,
                    mask    = wmask
                )
